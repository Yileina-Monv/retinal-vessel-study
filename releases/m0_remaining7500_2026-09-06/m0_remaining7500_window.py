...
try:
    ...
    if progress['step'] != 2500 or progress['checkpoint_sha256'] != digest(RUN / 'last.pt'):
        raise ValueError('Expected the intact 2500-step checkpoint')
    if (RUN / 'running.lock').exists():
        raise ValueError('Training is already locked')
    write(OUT / 'plan.json', dict(run_id=RUN.name, start_step=2500, stop_step=10000,
    ...
    write(OUT / 'window_result.json', dict(status='completed', segments=results,
          seconds=time.perf_counter()-started, temperature_recording=False))
except Exception:
