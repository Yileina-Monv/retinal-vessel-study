...
def main():
    started=time.perf_counter();guard=Budget();prefetch=None
    ...
    if args.max_seconds<120:raise ValueError('Allow startup and saving reserve')
    ticket=read(args.ticket);check_ticket(ticket)
    if ticket.get('workers') not in (1,2,4) or ticket.get('depth')!=ticket['workers']*2:raise ValueError('Invalid bounded prefetch profile')
    ...
    verified=verify()
    if verified['parent_release_id']!=ticket['release_id'] or verified['addon_id']!=ticket.get('addon_id') or verified['execution_id']!=ticket.get('execution_id'):raise ValueError('Job uses a different release')
    ...
    folder=ROOT/'runs/prefetch_round1'/ticket['run_id'];folder.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        deadline=started+args.max_seconds-60
        stop=min(ticket['steps'],args.stop_after_step if args.stop_after_step is not None else ticket['steps'])
        schedule=[plan[i*4:(i+1)*4] for i in range(step,stop)]
        prefetch=OrderedBatches(schedule,seed=seed,workers=ticket['workers'],depth=ticket['depth'],guard=guard)
        prefetch.__enter__()
        def save():
        ...
        while step<ticket['steps']:
            ...
            guard.check()
            tensors,batch_signatures,timing=next(prefetch)
            x,y,f,tube=[t.cuda() for t in tensors]
            ...
            step+=1;signatures.extend(batch_signatures);guard.cuda_sample()
        ...
        try:
            ...
            write(folder/'resources.json',guard.finish())
