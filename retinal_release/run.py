"""Ticketed M0 development run. Formal M1/M2 and cross-machine resume are disabled."""
import os
...
from retinal_release.data import DevelopmentDataset,masked_loss,fingerprint
...
def main():
    ...
    if ticket['kind']!='M0_development_only' or ticket['microbatch']!=4 or ticket['accumulation']!=1:raise ValueError('Unsupported job profile')
    verified=verify()
    if verified['release_id']!=ticket['release_id']:raise ValueError('Job uses a different release')
    ...
    folder=ROOT/'runs/development_v1'/ticket['run_id'];folder.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        if (folder/'last.pt').exists():
            ...
            del saved
        ds=DevelopmentDataset('train',seed=seed,cache_size=32)
        plan=[];epoch=0
        ...
        while step<ticket['steps']:
            ...
            x,y,f=[torch.stack([r[k] for r in items]).cuda() for k in ('image','mask','fov')]
            ...
            with torch.autocast('cuda',dtype=torch.float16):loss=masked_loss(model(x),y,f)
            ...
            history.append(dict(step=step,loss=float(loss.detach()),lr=lr,gradient_norm=float(norm)))
