...
101/118 图测量误差改善，但额外骨架增加，疾病相对正常组的测量偏移基本未消除。这是开发证据，不代表正式方法胜出。

**2026-09-15 进度补录：9 月 8 日统一阈值小测试与配对错误分析已完成。** [同等误报比较](outputs/matched_fp_20260908/测试报告.md)中，M1 细血管召回仍提高 1.5324 个百分点、密度 MARE 降低 0.7494 个百分点，但额外骨架代价仍在。[配对错误分析](outputs/paired_vessel_audit_20260908/分析报告.md)显示找回参考血管与新增无参考支持结构同时发生。下一步优先复核新增错误结构；C1 和新损失训练继续暂停。本次仅同步既有代码和结果，没有新训练或推理。
