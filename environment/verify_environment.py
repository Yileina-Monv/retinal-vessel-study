...
ARTIFACTS = ROOT / "environment" / "artifacts"
...
def check_gpu():
    ...
    assert arch in torch.cuda.get_arch_list(), f"Missing native architecture: {arch}"
...
def main():
    ...
    result = {"created_at": datetime.now(timezone.utc).isoformat(),
              "scope": "environment_only_synthetic_data", "status": "running",
              "python": sys.version, "executable": sys.executable,
              "platform": platform.platform(), "checks": {}}
    report = ROOT / "environment" / "verification.json"
    try:
